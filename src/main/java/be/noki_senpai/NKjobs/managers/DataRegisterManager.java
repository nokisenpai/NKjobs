...
public class DataRegisterManager
{
	...
	public DataRegisterManager(PlayerManager playerManager, QueueManager queueManager)
	{
		...
		}.runTaskTimerAsynchronously(NKjobs.getPlugin(), 0, 300 * 20);
	}
	...
	public void purgeOfflineBlocksTimer(Boolean notify)
	{
		...
		try
		{
			...
			req = "SELECT count(id) as count FROM " + DatabaseManager.table.BLOCKS + " WHERE time <= ? AND server_id = ?";
			...
			req = "DELETE FROM " + DatabaseManager.table.BLOCKS + " WHERE time <= ? AND server_id = ?";
			ps = bdd.prepareStatement(req);
			ps.setTimestamp(1, now);
			ps.setInt(2, SERVERID);
			ps.execute();
			...
			if(notify)
			{
				console.sendMessage(ChatColor.BLUE + NKjobs.PNAME + " " + count + " old blocks timer deleted.");
			}
		...
		{
			Bukkit.getConsoleSender().sendMessage(ChatColor.DARK_RED + NKjobs.PNAME + " Error while purging blocks timer data.");
			e.printStackTrace();
		}
	}
	...
	public boolean makeWorldsId()
	{
		...
		try
		{
			...
			for(World world : Bukkit.getWorlds())
			{
				...
				else
				{
					ps.close();
					resultat.close();
					Bukkit.getConsoleSender().sendMessage(ChatColor.DARK_RED + NKjobs.PNAME + " Error while getting worlds id.");
					return false;
				}
		...
		{
			Bukkit.getConsoleSender().sendMessage(ChatColor.DARK_RED + NKjobs.PNAME + " Error while getting blocks timer data.");
			e1.printStackTrace();
		}
		return true;
	}
	...
	public boolean loadBlocksTimer()
	{
		...
		try
		{
			...
			resultat.last();
			console.sendMessage(ChatColor.GREEN + NKjobs.PNAME + " Loaded " + resultat.getRow() + " blocks timer.");
	...
	public boolean loadFurnaces()
	{
		...
		try
		{
			...
			resultat.last();
			console.sendMessage(ChatColor.GREEN + NKjobs.PNAME + " Loaded " + resultat.getRow() + " registered furnaces.");
		...
		{
			Bukkit.getConsoleSender().sendMessage(ChatColor.DARK_RED + NKjobs.PNAME + " Error while getting registered furnaces data.");
			e1.printStackTrace();
		}
		return true;
	}
	...
	public boolean loadBrewingStands()
	{
		...
		try
		{
			...
			resultat.last();
			console.sendMessage(ChatColor.GREEN + NKjobs.PNAME + " Loaded " + resultat.getRow() + " registered brewing stands.");
		...
		{
			Bukkit.getConsoleSender().sendMessage(ChatColor.DARK_RED + NKjobs.PNAME + " Error while getting registered brewing stands data.");
			e1.printStackTrace();
		}
		return true;
	}
	...
	public boolean loadChunks()
	{
		...
		try
		{
			...
			resultat.last();
			console.sendMessage(ChatColor.GREEN + NKjobs.PNAME + " Loaded " + resultat.getRow() + " explored chunks.");
		...
		{
			Bukkit.getConsoleSender().sendMessage(ChatColor.DARK_RED + NKjobs.PNAME + " Error while getting explored chunks data.");
			e1.printStackTrace();
		}
		return true;
	}
	...
	public void saveChunks()
	{
		if(newChunks.size() > 0)
		{
			...
			String req = "INSERT INTO " + DatabaseManager.table.CHUNKS + " ( server_id, world_id, x, z, players) VALUES ";
	...
	public void registerFurnace(Location location, NKPlayer player)
	{
	...
	public void registerBrewingStand(Location location, NKPlayer player)
	{
		queueManager.addToQueue(new Function()
		{
			@Override
