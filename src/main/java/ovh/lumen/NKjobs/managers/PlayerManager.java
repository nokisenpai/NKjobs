...
{
	// Players datas
	private Map<String, NKWorker> players = null;
	private ConsoleCommandSender console = null;
	private JobManager jobManager = null;

	public PlayerManager(JobManager jobManager)
	{
		this.players = new TreeMap<String, NKWorker>(String.CASE_INSENSITIVE_ORDER);
		...
		}.runTaskTimerAsynchronously(Main.getPlugin(), 0, 120 * 20);
		...
		}.runTaskTimerAsynchronously(Main.getPlugin(), 0, 3 * 20);
	}

	public void loadPlayer()
	{
		// Get all connected players
		Bukkit.getOnlinePlayers().forEach(player -> players.put(player.getName(), new NKWorker(player.getUniqueId(), jobManager)));
	}
	...
	{
		players.put(player.getName(), new NKWorker(player.getUniqueId(), jobManager));
	}
	...
	public void saveAll()
	{
		if(players.size() > 0)
		{
			...
			String req = "INSERT INTO " + DatabaseManager.Tables.PLAYER_JOBS + " ( player_id, job_id, lvl, xp, xp_goal, xp_total, old, xp_day, time) VALUES ";
			boolean ok = false;
			for(Map.Entry<String, NKWorker> player : players.entrySet())
			{
	...
	public void giveGain()
	{
		if(players.size() > 0)
		{
			for(Map.Entry<String, NKWorker> player : players.entrySet())
			{
				...
				{
					Main.getPlugin().getEconomy().depositPlayer(Bukkit.getOfflinePlayer(player.getValue().getUuid()), player.getValue().getTmpMoney());
					Bukkit.getPlayer(player.getValue().getUuid()).spigot().sendMessage(ChatMessageType.ACTION_BAR, new ComponentBuilder(
							ChatColor.YELLOW + "+" + Main.getPlugin().getEconomy().format(player.getValue().getTmpMoney()) + ChatColor.BLUE +  "   +" + Formatter.formatMoney(player.getValue().getTmpExp()) + " XP").create());
	...
	public NKWorker getPlayer(String playerName)
	{
		...
		else
		{
			...
			try
			{
				...
				if(resultat.next())
				{
					return new NKWorker(UUID.fromString(resultat.getString("uuid")), jobManager);
				}
			}
			catch(SQLException e1)
			{
				Bukkit.getConsoleSender().sendMessage(ChatColor.DARK_RED + Main.PNAME + " Error while getting a player.");
			}
		}
		return null;
	}
	...
	private void onlineJoinJob(NKWorker player, String jobName, boolean crossServer)
	{
	...
	private void offlineJoinJob(NKWorker player, String jobName)
	{
		Connection bdd = null;
		PreparedStatement ps = null;
		String req = null;
		...
		{
			bdd = DatabaseManager.getConnection();
			req = "INSERT INTO " + DatabaseManager.Tables.PLAYER_JOBS
	...
	private void crossServerJoinJob(final String playerName, String jobName)
	{
		...
		if(server != null)
		{
			...
			{
				Bukkit.getConsoleSender().sendMessage(ChatColor.DARK_RED + Main.PNAME + " " + playerName
						+ " est connecté(e) sur un autre serveur. Utilisez la console de ce serveur pour lui ajouter un métier.");
			}
		}
	}

	public void joinJob(NKWorker player, String jobName, boolean crossServer)
	{
		if(!crossServer)
		{
	...
	private void onlineLeaveJob(NKWorker player, String jobName, boolean crossServer)
	{
	...
	private void offlineLeaveJob(NKWorker player, String jobName)
	{
		Connection bdd = null;
		PreparedStatement ps = null;
		String req = null;
		...
		try
		{
			...
			req = "UPDATE " + DatabaseManager.Tables.PLAYER_JOBS
	...
	public void leaveJob(NKWorker player, String jobName, boolean crossServer)
	{
		if(!crossServer)
		{
	...
	private void onlineLeaveAllJob(NKWorker player, boolean crossServer)
	{
	...
	private void offlineLeaveAllJob(NKWorker player)
	{
		Connection bdd = null;
		PreparedStatement ps = null;
		String req = "INSERT INTO " + DatabaseManager.Tables.PLAYER_JOBS + " ( player_id, job_id, lvl, xp, xp_goal, xp_total, old) VALUES ";
	...
	public void leaveAllJob(NKWorker player, boolean crossServer)
	{
		if(!crossServer)
		{
	...
	private void onlineSwitchJob(NKWorker player, String jobName, boolean crossServer)
	{
		// Remove all jobs
		for(Map.Entry<String, PlayerJob> jobs : player.getJobs().entrySet())
		{
	...
	private void offlineSwitchJob(NKWorker player, String jobName)
	{
		...
		try
		{
			...
			{
				// Leave all job
				req = "INSERT INTO " + DatabaseManager.Tables.PLAYER_JOBS + " ( player_id, job_id, lvl, xp, xp_goal, xp_total, old) VALUES ";
	...
	public void switchJob(NKWorker player, String jobName, boolean crossServer)
	{
		if(!crossServer)
		{
	...
	private void onlineExpJob(NKWorker player, String jobName, Double amount, String subCommand)
	{
		switch(subCommand)
		{
	...
	private void offlineExpJob(NKWorker player, String jobName, Double amount, String subCommand)
	{
		Connection bdd = null;
		PreparedStatement ps = null;
		String req = null;
	...
	public void expJob(NKWorker player, String jobName, Double amount, String subCommand, boolean crossServer)
	{
		if(!crossServer)
		{
	...
	private void onlineLevelJob(NKWorker player, String jobName, int amount, String subCommand)
	{
		switch(subCommand)
		{
	...
	private void offlineLevelJob(NKWorker player, String jobName, int amount, String subCommand)
	{
		Connection bdd = null;
		PreparedStatement ps = null;
		String req = null;
	...
	public void levelJob(NKWorker player, String jobName, int amount, String subCommand, boolean crossServer)
	{
		if(!crossServer)
		{
	...
	public Map<String, NKWorker> getPlayers()
	{
		return players;
	}
}
