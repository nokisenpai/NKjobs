...
import ovh.lumen.NKjobs.managers.*;
...
public class NKWorker
{
	...
	public NKWorker(UUID uuid, JobManager jobManager) //throws SQLException
	{
		this.uuid = uuid;
		...
		try
		{
			...
			{
				Bukkit.getConsoleSender().sendMessage(ChatColor.DARK_RED + Main.PNAME + " Error while setting a player. (#1)");
			}

			ps.close();
			resultat.close();

			req = "SELECT name, job_id, lvl, xp, xp_goal, xp_total, xp_day, time, p.old FROM " + DatabaseManager.Tables.PLAYER_JOBS + " p " + "LEFT JOIN "
					+ DatabaseManager.Tables.JOBS + " j " + "ON p.job_id = j.id WHERE player_id = " + id;
			ps = bdd.prepareStatement(req);
			resultat = ps.executeQuery();
			...
			req = "SELECT COUNT(id) AS count FROM " + DatabaseManager.Tables.FURNACES + " WHERE player_id = ? AND server_id = ?";
			ps = bdd.prepareStatement(req);
			ps.setInt(1, id);
			...
			req = "SELECT COUNT(id) AS count FROM " + DatabaseManager.Tables.BREWINGSTANDS + " WHERE player_id = ? AND server_id = ?";
			ps = bdd.prepareStatement(req);
			ps.setInt(1, id);
			ps.setInt(2, DataRegisterManager.SERVERID);
			resultat = ps.executeQuery();
			...
			req = "SELECT id, item FROM " + DatabaseManager.Tables.ITEMS + " WHERE player_id = ?";
			ps = bdd.prepareStatement(req);
			ps.setInt(1, getId());
			resultat = ps.executeQuery();
		...
		{
			Bukkit.getConsoleSender().sendMessage(ChatColor.DARK_RED + Main.PNAME + " Error while setting a player. (Error#3.004)");
			e1.printStackTrace();
		}
	...
	public void save()
	{
		...
		try
		{
			...
			{
				req = "UPDATE " + DatabaseManager.Tables.PLAYER_JOBS + " SET lvl = ? , xp = ? , xp_goal = ? , xp_total = ? , old = false WHERE player_id = ? AND job_id = ?";
				ps = bdd.prepareStatement(req);
	...
	public void checkLostRewardedItems(Inventory inventory)
	{
		...
		if(!ids.isEmpty())
		{
			...
			{
				bdd = DatabaseManager.getConnection();
				req = "DELETE FROM " + DatabaseManager.Tables.ITEMS + " WHERE id IN (";
			...
			{
				Bukkit.getConsoleSender().sendMessage(ChatColor.DARK_RED + Main.PNAME + " Error while deleting rewarded item.");
				e.printStackTrace();
			}
		}
	}
	...
	public void saveRewardedItem()
	{
		...
		{
			bdd = DatabaseManager.getConnection();
			req = "INSERT INTO " + DatabaseManager.Tables.ITEMS + " ( player_id , item ) VALUES ";
		...
		{
			Bukkit.getConsoleSender().sendMessage(ChatColor.DARK_RED + Main.PNAME + " Error while adding rewarded item.");
			e.printStackTrace();
		}
	}
